...
while true; do
    CONFIGURATION='Debug'
    read -p "Build Debug or Release (d/r) [d]? " dr
    case $dr in
        [Rr]* )
...
then
    while true; do
        read -p "Rebuild SDL2 library (iPhone Simulator + ${CONFIGURATION}) [no]? " yn
        case $yn in
            [Yy]* )
                rm -rf SDL/Xcode-iOS/SDL/build
                xcodebuild -project SDL/Xcode-iOS/SDL/SDL.xcodeproj -sdk iphonesimulator -configuration ${CONFIGURATION}
                if [ $? -ne 0 ]
                then
                    echo "[E] Build failed: couldn't create makefiles"
                    return $?
                else
                    echo "[X] SDL2 library created (iPhone Simulator + ${CONFIGURATION})"
                fi
                break;;
            * ) break;;
        esac
    ...
    while true; do
        ...
        case $yn in
            ...
            * )
                ...
                xcodebuild -project SDL_image/Xcode-iOS/SDL_image.xcodeproj -sdk iphonesimulator -configuration ${CONFIGURATION}
                if [ $? -ne 0 ]
                then
                    echo "[E] Build failed: cound't create SDL2_image"
                    return $?
                else
                    echo "[X] SDL2_image library created (iPhone Simulator + ${CONFIGURATION})"
                fi
                break;;
        esac
    done
    ...
    while true; do
        ...
        case $yn in
            ...
            * )
                ...
                xcodebuild -project SDL_ttf/Xcode-iOS/SDL_ttf.xcodeproj -sdk iphonesimulator -configuration ${CONFIGURATION}
                if [ $? -ne 0 ]
                then
                    echo "[E] Build failed: cound't create SDL2_ttf"
                    return $?
                else
                    echo "[X] SDL2_ttf library created (iPhone Simulator + ${CONFIGURATION})"
                fi
                break;;
        esac
    done
    ...
    while true; do
        ...
        case $yn in
            ...
            * )
                ...
                xcodebuild -project SDL_mixer/Xcode-iOS/SDL_mixer.xcodeproj -sdk iphonesimulator -configuration ${CONFIGURATION}
                if [ $? -ne 0 ]
                then
                    echo "[E] Build failed: cound't create SDL2_mixer"
                    return $?
                else
                    echo "[X] SDL2_mixer library created (iPhone Simulator + ${CONFIGURATION})"
                fi
                break;;
        esac
    done
